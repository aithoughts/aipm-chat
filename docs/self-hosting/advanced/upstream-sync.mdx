---
title: LoiChat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for LoiChat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest LoiChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running LoiChat Container

Assuming the LoiChat container is named `lobe-chat`, use the following commands to stop and remove the currently running LoiChat container:
...
### Pull the Latest LoiChat Image

Use the following command to pull the latest Docker image for LoiChat:
...
Redeploy the LoiChat container using the newly pulled image:
...
No need to worry, you won't. All of LobeChat's chat records are stored in your local browser. Therefore, when redeploying LoiChat using Docker, your chat records will not be lost.
