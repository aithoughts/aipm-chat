---
title: 在 LoiChat 中配置 Cloudflare Zero Trust 身份验证服务
description: >-
  学习如何在 LoiChat 中配置 Cloudflare Zero Trust 身份验证服务，包括创建提供程序、配置环境变量和部署
...
我们现在默认您已经了解了如何使用 Cloudflare Zero Trust 平台且假设您的 LoiChat 实例部署在 `https://chat.example.com` 中。
...
在 `Application` 文本框内填入应用名称，如：`LoiChat SSO`，然后点击 `Select OIDC` 后点击 `Add applicaiton`

![image](https://github.com/user-attachments/assets/16cd9aef-c87b-48a4-95c0-b666082e7515)

至此您已成功在 Clouflare Zero Trust 中创建了一个名为 `LoiChat SSO` 的 SaaS 应用。
...
最后我们将页面往下滚动，您将需要记录以下三个值 `Client secret`, `Client ID` 及 `Issuer` 以备后续部署 LoiChat 环境变量使用。

![image](https://github.com/user-attachments/assets/2dd3cde5-fa0d-4f52-b82b-28d9e89379a0)

### 配置环境变量

在部署 LoiChat 时，你需要配置以下环境变量：
