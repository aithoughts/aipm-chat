...
We assume you are already familiar with using the Cloudflare Zero Trust platform and that your LoiChat instance is deployed at `https://chat.example.com`.
...
In the `Application` text box, enter the application name, such as `LoiChat SSO`. Then click `Select OIDC`, followed by clicking `Add application`.

![image](https://github.com/user-attachments/assets/16cd9aef-c87b-48a4-95c0-b666082e7515)

At this point, you have successfully created a SaaS application named `LoiChat SSO` in Cloudflare Zero Trust.
...
Finally, scroll down the page and record the following three values: `Client secret`, `Client ID`, and `Issuer`. You will need these for setting the environment variables when deploying LoiChat.
...
<Callout type={'info'}>
  After a successful deployment, users will be able to use LoiChat by authenticating with the users
