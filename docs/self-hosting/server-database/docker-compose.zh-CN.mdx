...
description: 学习如何使用 Docker Compose 部署 LoiChat 服务，包括各种服务的配置教程。
tags:
  - Docker Compose
  - LobeChat
  - Docker 容器
...
# 使用 Docker Compose 部署 LoiChat 服务端数据库版本
...
<Callout type="info">
  本文已经假定你了解了 LoiChat 服务端数据库版本（下简称 DB
  版）的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 LoiChat DB
...
<Callout type="warning">
  ...
  部署 LoiChat 时，你不能使用 Clerk 作为登录鉴权服务。

如果你确实需要 Clerk 作为登录鉴权服务，你可以考虑使用 Vercel 部署或者自行构建镜像。

</Callout>

一般来讲，想要完整的运行 LoiChat 数据库版本，你需要至少拥有如下四个服务

- LoiChat 数据库版本自身
- 带有 PGVector 插件的 PostgreSQL 数据库
- 支持 S3 协议的对象存储服务
- 受 LoiChat 支持的 SSO 登录鉴权服务

这些服务可以通过自建或者在线云服务组合搭配，以满足不同层次的部署需求。

本文中，我们提供了完全基于开源自建服务的 Docker Compose 配置，你可以直接使用这份配置文件来启动 LoiChat 数据库版本，也可以对之进行修改以适应你的需求。

我们默认使用 [MinIO](https://github.com/minio/minio) 作为本地 S3 对象存储服务，使用 [Logto](https://github.com/logto-io/logto) 作为本地鉴权服务。

## 快速启动

为方便快速上手，这一章使用 `docker-compose/local` 目录中的 docker-compose 配置文件，启动后的 LoiChat 应用运行在 `http://localhost:3210`，可在本地运行。
...
至此，你已经成功部署了 LoiChat 数据库版本，你可以通过 `http://localhost:3210` 访问你的 LoiChat 服务。
...
- `lobe.example.com`：为你的 LoiChat 服务端域名，需要反向代理到 LoiChat 服务端口，默认为 `3210`
...
配置文件包括 `.env` 和 `docker-compose.yml` 两个文件，其中 `.env` 文件用于配置 LoiChat 的环境变量，`docker-compose.yml` 文件用于配置 Postgres、MinIO 和 Logto 服务。
...
<Callout type="warning">
  请记得注意配置对应登录鉴权服务提供商的 CORS 跨域配置，以确保 LoiChat 能够正常访问登录鉴权服务。
...
<Callout type="warning">
  请记得注意配置对应 S3 服务商的 CORS 跨域配置，以确保 LoiChat 能够正常访问 S3 服务。
...
至此，你已经成功部署了 LoiChat 数据库版本，你可以通过 `https://lobe.example.com` 访问你的 LoiChat 服务。
