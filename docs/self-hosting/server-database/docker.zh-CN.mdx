---
title: 使用 Docker 部署 LoiChat 数据库
description: 详细步骤教你如何在 Docker 中部署 LoiChat 服务端数据库。
tags:
  - Docker
  - LobeChat
  - 数据库部署
...
以下是在 Linux 服务器上部署 LoiChat DB 版的流程：
...
<Callout type="info">
  pgvector 插件为 Postgres 提供了向量搜索的能力，是 LoiChat 实现 RAG 的重要构件之一。
</Callout>
...
LoiChat 的 DB 版也支持直接在本地的 Mac/Windows 本地使用。
