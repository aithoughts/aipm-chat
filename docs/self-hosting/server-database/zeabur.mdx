---
title: Deploying LoiChat Database on Zeabur
description: >-
  Learn how to deploy LoiChat on Zeabur with ease. Follow the provided steps to
...
# Deploying LoiChat Database on Zeabur

<Callout type="info">
  This article assumes that you are familiar with the basic principles and processes of deploying
  the LoiChat server database version, so it only includes content related to core environment
...
- LoiChat database version.

## Deploying on Zeabur

Here is the process for deploying the LoiChat server database version on Zeabur:

<Steps>

### Go to the template page on Zeabur

Go to the [LoiChat Database template page](https://zeabur.com/templates/RRSPSD) on Zeabur and click on the "Deploy" button.
...
- LoiChat Domain: A free subdomain with `.zeabur.app` suffix.
...
After you fill all the required environment variables, select a region where you want to deploy your LoiChat Database and click on the "Deploy" button.
...
Fill in those variables into your LoiChat service on Zeabur, here is a more detailed guide for [editing environment variables on Zeabur](https://zeabur.com/docs/deploy/variables).
...
### Access your LoiChat Instance

Press on the `LobeChat-Database` and you can see the public domain you just created, click on it to access your LoiChat Database.
