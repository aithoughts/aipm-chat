---
title: Deploying LoiChat Database with Docker
description: >-
  Learn how to deploy the LoiChat server database version using Docker on Linux
  and local machines.
tags:
  - LobeChat
  - Docker
...
<Callout type="info">
This article assumes that you are familiar with the basic principles and processes of deploying the LoiChat server database version, so it only includes content related to core environment variable configuration. If you are not familiar with the deployment principles of the LoiChat server database version, please refer to [Deploying Server Database](/docs/self-hosting/server-database) first.
</Callout>
...
Here is the process for deploying the LoiChat server database version on a Linux server:
...
<Callout type="info">
The pgvector plugin provides vector search capabilities for Postgres, which is an important component for LoiChat to implement RAG.
</Callout>
...
The data version of LoiChat also supports direct use on a local Mac/Windows machine.
