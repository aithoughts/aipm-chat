---
title: 在 Vercel 上部署 LoiChat 的服务端数据库版本
...
该部分最后需要添加 APP_URL 环境变量，用于指定LoiChat 应用的 URL 地址。
...
<Callout type={'warning'}>`S3_ENDPOINT`必须删除其路径，否则会无法访问所上传文件</Callout>
    ...
    你需要获取 S3 的访问密钥，以便 LoiChat 的服务端有权限访问 S3 存储服务。在 R2 中，你可以在账户详情中配置访问密钥：
...
通过上述步骤之后，我们应该就完成了服务端数据库的配置。接下来我们可以将 LoiChat 部署到 Vercel 上，然后访问你的 Vercel 链接，验证服务端数据库是否正常工作。
