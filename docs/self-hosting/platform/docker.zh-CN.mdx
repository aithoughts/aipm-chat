...
description: 学习如何使用 Docker 部署 LoiChat 服务，包括安装 Docker 容器环境和使用指令一键启动服务。详细说明如何配置环境变量和使用代理地址。
tags:
  - Docker
  - LobeChat
  - 部署指引
...
我们提供了 [Docker 镜像][docker-release-link]，供你在自己的私有设备上部署 LoiChat 服务。
...
API Key 是使用 LoiChat 进行大语言模型会话的必要信息，本节以 OpenAI 模型服务商为例，简要介绍获取 API Key 的方式。
...
将此 API Key 填写到 LoiChat 的 API Key 配置中，即可开始使用。
