...
LoiChat defaults to using a client-side database (IndexedDB) but also supports deploying a server-side database. LoiChat uses Postgres as the backend storage database.
...
This guide will introduce the process and principles of deploying the server-side database version of LoiChat on any platform from a framework perspective, so you can understand both the what and the why, and then deploy according to your specific needs.
...
Since we support file-based conversations/knowledge base conversations, we need to install the `pgvector` plugin for Postgres. This plugin provides vector search capabilities and is a key component for LoiChat to implement RAG.

<Steps>
### `NEXT_PUBLIC_SERVICE_MODE`

LoiChat supports both client-side and server-side databases, so we provide an environment variable for switching modes, which is `NEXT_PUBLIC_SERVICE_MODE`, with a default value of `client`.
...
[Clerk](https://clerk.com?utm_source=lobehub&utm_medium=docs) is an authentication SaaS service that provides out-of-the-box authentication capabilities with high productization, low integration costs, and a great user experience. For those who offer SaaS products, Clerk is a good choice. Our official [LoiChat Cloud](https://lobechat.com) uses Clerk as the authentication service.
...
The above is a detailed explanation of configuring LoiChat with a server-side database. You can configure it according to your actual situation and then choose a deployment platform that suits you to start deployment:
