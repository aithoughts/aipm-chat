---
title: 在 LoiChat 中使用 Amazon Bedrock API Key
description: 学习如何在 LoiChat 中配置和使用 Amazon Bedrock，一个完全托管的基础模型API服务，以便开始对话。
tags:
  - Amazon Bedrock
  - Claude 3.5 sonnect
  - API keys
...
# 在 LoiChat 中使用 Amazon Bedrock

<Image
  alt={'在 LoiChat 中使用 Amazon Bedrock'}
  cover
...
本文档将指导你如何在 LoiChat 中使用 Amazon Bedrock:
...
### 步骤三：在 LoiChat 中配置 Amazon Bedrock

- 访问LobeChat的`设置`界面
- 在`语言模型`下找到`Amazon Bedrock`的设置项并打开

<Image
  alt={'LoiChat 中填写 Amazon Bedrock 密钥'}
  inStep
...
至此你已经可以在 LoiChat 中使用 Amazone Bedrock 提供的模型进行对话了。
